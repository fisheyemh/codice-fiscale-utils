const moment = require('moment'),
    Omocode = require('./omocode.enum'),
    Gender = require('./gender.enum'),
    BirthMonth = require('./birthMonth.enum'),
    VALIDATOR = require('./validator.const'),
    DIACRITICS = require('./diacritics.const'),
    Belfiore = require('./belfiore'),
    CheckDigitizer = require('./checkDigitizer');
...
class Parser {
    ...
    static cfDeomocode (codiceFiscale) {
        ...
        const checkBitmap = (offset) => Boolean(2 ** offset & this.OMOCODE_BITMAP);
    ...
    static cfToSurname (codiceFiscale) {
        ...
        const surnameCf = codiceFiscale.substr(0, 3),
            ...
            [cons = ''] = surnameCf.match(new RegExp(`^[${VALIDATOR.CONSONANT_LIST}]{1,3}`, 'ig')) || [],
            [vow = ''] = surnameCf.match(new RegExp(`[${VALIDATOR.VOWEL_LIST}]{1,3}`, 'ig')) || [],

            matchingLength = cons.length + vow.length;
    ...
    static cfToBirthYear (codiceFiscale) {
        ...
        const current2DigitsYear = parseInt(moment().format('YY')),
            ...
            century = (birthYear > current2DigitsYear) * 100;
        return moment().subtract(current2DigitsYear - birthYear + century, 'years').
            year();
    ...
    static surnameToCf (surname) {
        ...
        const noDiacriticsSurname = this.removeDiacritics(surname),
            consonants = (noDiacriticsSurname.match(new RegExp(`[${VALIDATOR.CONSONANT_LIST}]+`, 'ig')) || []).join(''),
            vowels = (noDiacriticsSurname.match(new RegExp(`[${VALIDATOR.VOWEL_LIST}]+`, 'ig')) || []).join(''),

            partialCf = `${consonants + vowels}X`.substr(0, 3);
    ...
    static nameToCf (name) {
        ...
        const noDiacriticsName = this.removeDiacritics(name),
            consonants = (noDiacriticsName.match(new RegExp(`[${VALIDATOR.CONSONANT_LIST}]+`, 'ig')) || []).join('');
     ...
     * @memberof CodiceFiscaleUtils.Parser
     */
    static yearToCf (strIntYear) {

        const year = parseInt(strIntYear, 10);
    ...
    static dateGenderToCf (...args) {
        ...
        const cfYear = this.yearToCf(date.getFullYear()),
            cfMonth = this.monthToCf(date.getMonth()),
            cfDayGender = this.dayGenderToCf(date.getDate(), gender);
    ...
    static placeToCf (...args) {
        ...
        const targetDate = this.parseDate(...args.slice(0, args.length - 1)),

            [
                name,
                province
            ] = args.filter((input) => typeof input === 'string');
        if (!province) {

            return (Belfiore.active(targetDate).findByName(name) || {}).belfioreCode;

        }
        const results = Belfiore.searchByName(name).toArray().
            filter((place) => province.trim().toUpperCase() === place.province);
        if (results.length === 1) {
